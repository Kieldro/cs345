int main () {
    ...
    list dbRes = [ (name, date, city) | 
                  ...
                  (d2, _, city) <- dept, d1 == d2];
                  
    println("Query Result: ", dbRes);
    
    println("Expense: ", calcExpense(emp));
    
    
    // Testing the Lambda expressions:
    bool eqRes = equivalence2( (\ x, y -> !(x || y)), (\ x, y -> !x && !y));
    println("Result1: ", eqRes);
    
    eqRes = equivalence2( (\ x, y -> x || y), (\ x, y -> !x && y));
    println("Result2: ", eqRes);
}


bool equivalence2( (object, object, object) a,
